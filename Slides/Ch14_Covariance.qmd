...
abline(v=with(params, cor(V0, r)))
text(x=with(params, cor(V0, r)), y=1.5, adj=c(-0.05, 1), labels = "Emperical rho")
...
```{r}
df1 <- data.frame(V0 = colMeans(post1$a), 
                  r = colMeans(post1$b)
)
df2 <- data.frame(V0 = colMeans(post2$a), 
                  r = colMeans(post2$b)
)

plot(r ~ V0, data = params, pch=20)
points(df1$V0, df1$r, col = "red")
points(df2$V0, df2$r, col = "blue")

for ( i in 1:nS ){
  lines( c(params$V0[i],df1$V0[i]) ,
         c(params$r[i],df1$r[i]), col = "red") 
    lines( c(params$V0[i],df2$V0[i]) ,
