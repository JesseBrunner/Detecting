...
```{r}
#| out-width: 100% #| fig-format: svgin
#| 
curve(L2(x, 
...
for(i in 2:2000){
  curve(L2(x, 
           eta=sqrt(rexp(1, rate=1)), 
           rho=sqrt(rexp(1, rate=1)), 
           delta=0), add=T, col = col.alpha("grey", alpha = 0.1))
}
...
```{r}
dens(post1$b, adj = 0.75, xlab="values of b")
dens(post2$b, adj = 0.75, add=T, col = "blue")
