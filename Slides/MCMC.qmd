...
Choose a _proposal_ for a new value on the chain.
  - Our proposal distribution (blue) is centered on our last guess. 
...
$$
r = \frac{\text{Pr}(x|\theta_p)\text{Pr}(\theta_p)}{\text{Pr}(x|\theta_{t-1})\text{Pr}(\theta)} = \frac{`r round(posterior(2.5),3)`}{`r round(posterior(1.9),3)`} = `r round(posterior(2.5)/posterior(1.9),3)`
$$
...
If  $r < U$, reject and try a new proposal.
...
```{r}
hist(theta, breaks = 50, xlab=expression(theta))
```
...
Assumes a continuous likelihood surface, so _no discrete parameters_! 
