...
First off, congratulations! We've simulated data! This is one example of data we could get from our experimental design. But now that we have the code, shouldn't we futz with things a bit? Absolutely!
...
where $h$ represents handling time, but everything else is the same. Indeed, we are really only changing the relationship (part 2, above), so this should be quick. 
