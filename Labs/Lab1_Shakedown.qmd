...
Given all of this, my goals for this lab are to:
  ...
  * Introducing some base R functions we will see used frequently in the text
...
```
And for fun, let's make `Date` an actual date. The package `lubridate` has some handy functions, but in base R this is:
```{r}
...
Full disclosure, I hate plotting in base R anymore! I can't even figure out how to add a meaningful legend anymore. This is much simpler with `ggplot2`, if you know how that works:
```{r}
library(ggplot2)
...
```
First, be sure you understood what's happening here (i.e., how I'm subsetting the `smammals` data set by those where the condition is true and then getting a summary of this reduced data set). (And yes, this would be much easier to follow in the Tidyverse! But not all things are.)

We could also use the `tapply()` function:
```{r}
tapply(X = smammals$Body, 
       INDEX = smammals$Species, 
       FUN = summary)
```
I like to think of this function as the "table" apply function because it first organizes our data like a data table, according to whatever index we've given it, and then applies the function of our choice. Or, more specifically, it applies the function specified by `FUN` to the vector (or similar) of data specified by `X`, which is split into groups according to the variable specified by `INDEX`. We will see and use several `apply` functions in this class (mostly `apply()`, which we will see below, and `sapply()`). They are _not_ always very intuitive, but they are very powerful and McElreath uses them frequently, so it behooves us to get used to them. (Also note, in this case it would be easy to accomplish the same thing in the Tidyverse, but there are several cases we'll see where it is _not_ easy or transparent to do so.)
...
data.frame(bySpp) %>% 
  ...
  pivot_longer(names_to = "Species", 
               values_to = "Captures", 
               cols = everything()) %>% 
  ggplot(., aes(x=Captures, fill = Species) ) + 
...
For instance, what do you think this will do?
```{r}
#| output: false
apply(smammals, 
      MARGIN = 2, 
      FUN = min, na.rm = TRUE)
```
Really. Take a second and think before moving on...
...
Before moving on, make sure you understand what those functions _do_. What happens if we didn't that that `na.rm=TRUE` in the command? Why?
