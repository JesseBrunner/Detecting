...
```{r}
#| eval: false
coeftab_plot(coeftab(mF, mT, mFT, mFTC), pars = c("bf", "bt"))
```
...
adjustmentSets(virus2, 
               exposure = c("Food", "Temp"), 
               outcome = "Virus", 
               effect = "total", 
               type = "all")
## Not changed!
...
mFTCE <- quap(
  alist(
    Virus ~ dnorm(mu, sigma), 
    mu <- a + bf*Food + bt*Temp + bc*Condition + be*En,
    # priors
    a ~ dnorm(3, 3), 
    bf ~ dnorm(0, 1), 
    bt ~ dnorm(0, 1), 
...
precis(mFTCE)
plot(coeftab(mFT, mFTC, mFTCE), par = c("bf", "bt"))
# The food coefficients is near zero because we've conditioned on its pipe
# The temperature coefficient just represents the direct effect b/c we conditioned on the intermediary of En in the pipe.
```
...
coordinates(virus3) <- list(
  x=c(Food=1, En=4.5, Condition=6, Temp=1,  Virus = 9),
  y=c(Food=1, En=4.5, Condition=2.5, Temp=9,  Virus = 9))
drawdag(virus3)

impliedConditionalIndependencies(virus3)
# No conditional independencies

adjustmentSets(virus3, exposure = "Food", 
               outcome = "Virus", 
               effect="total", 
               type = "all")
# Must condition on temperature to estimate the total effect of food on Virus
adjustmentSets(virus3, exposure = "Temp", 
               outcome = "Virus", 
               effect="total", 
               type = "all")
# Still no adjustments for the total effect of temperature

adjustmentSets(virus3, exposure = c("Temp"), 
               outcome = "Virus", 
               effect="direct")
# Still not possible to find the direct effect of temp on virus if we don't observe En
```
...
```{r}
# step 3
plot(coeftab(mT, mT2, mT3), pars=c("bt"))
# We can see that the effect size goes from positive to negative when we include food. 
# This is presumably because we are conditioning on the pipe from Temp to Food to En to Virus, blocking this effect. Moreover, the effect of temperature on En is strongly negative (-0.75 vs the 0.5 of b_TE) so the leftover effect of virus is more negative than positive. Plus, biases get weird...
```
...
# without the temperature effect
mF1 <- quap(
  alist(
    Virus ~ dnorm(mu, sigma), 
    mu <- a + bf*Food, 
...
# with temperature, but also condition
mF2 <- quap(
  alist(
    Virus ~ dnorm(mu, sigma), 
    mu <- a + bf*Food + bt*Temp + bc*Condition, 
...
plot(coeftab(mF, mF1, mF2), pars=c("bf"))
# without temperature (mF1), we get a much smaller estimated effect of food. This is b/c the estimate of bf absorbs the negative influence of temperature, with which it is strongly correlated.
# with temperature and condition, we get a slightly biased estimate with more uncertainty. I think it just muddies the waters. 
```
