...
compare(ma, ma_t, mb, mb_t, mc, mc_t, func="PSIS")
```

```{r}
# explain the pattern

# Better at fitting: With a less surprised distribution, our models are not pulled as much by more extreme data points. They tend to focus on the "regular features" of the data rather than overfitting. This can make them better at predicting new observations. 

# Worse at fitting: With a less surprised distribution, our models may not be learning as much from the data about the "regular features" as they otherwise might. In some cases unexpected values might really be coming from some other process and so we wouldn't want them to pull our model toward them so much, but in this case our observations all come from the same process. There are no "outliers" in our data!
```
...
```{r}
# compare estimated predictive ability
compare(ma, ma_w, mb, mb_w, mc, mc_w, func="PSIS")
compare(ma, ma_w, mb, mb_w, mc, mc_w, func="WAIC")
```

```{r}
# explain the pattern

# With enough data the priors don't matter much, which is mostly what we're seeing (in my data). However, in general,  models with wide priors are pulled more by the extreme observations because they do not weight them lower, a priori, than the models with narrower priors. This can make them worse at out-of-sample prediction. 
```
