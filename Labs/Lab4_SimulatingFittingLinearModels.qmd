...
# A simple model of height by weight
Let me propose this form and priors for a model of height against weight:
...
\end{align}
$$
where $y_i$ is the height of individual $i$ and $x_i$ is their weight. 
...
Given the correlation we observed, let me proposal a mathematically equivalent model structure: center your $x$-axis on the mean of those $x$ values. That is, let $wt_i =  w_i - \bar{w}_i$ and use $wt$ as your predictor variable. 
```{r}
df$wt <- df$weight - mean(df$weight)
summary(df$wt)
```
...
\begin{align}
y_i & \sim \text{Normal}(\mu_i, \sigma) \\
\mu_i & = \alpha + \beta  (\log(x_i) - \overline{\log(x)}) \\
\alpha & \sim \text{Normal}(130,20) \\
\beta & \sim \text{Normal}(40,20) \\
\sigma & \sim \text{Exponential}(1)
\end{align}
$$
...
Fifth, graph the relationship between height and weight predicted this new model, as well as the actual observations. 

```{r}
# plot on the log-weight scale
